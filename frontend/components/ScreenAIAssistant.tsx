...
export default function ScreenAIAssistant({ onNavigate }: ScreenAIAssistantProps) {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const toolsList = res.tools_executed || res.toolsExecuted || (res.toolExecuted ? [res.toolExecuted] : []);
      const hasCreateApt = toolsList.some((t: any) => (t?.toolName || t?.name) === "create_appointment");
      const hasCreateTask = toolsList.some((t: any) => (t?.toolName || t?.name) === "create_task");
      ...
      if (hasCreateApt) {
        actionPlan = {
          ...
          actions: [
            "Reserved slot in Organization Calendar",
            "Recorded appointment in database",
            "Customer record linked"
          ],
          status: "executed" as const
        };
      } else if (hasCreateTask) {
        actionPlan = {
          title: "Task Automation Scheduled",
          trigger: "AI CRM Task Tool",
          actions: [
            "Created task in organization tasks board",
            "Due date configured"
          ],
          status: "executed" as const
        };
        ...
        onExecute={async () => {
          try {
            await api.createWorkflow({
              name: activePlanModal.title || "AI Action Plan",
              trigger_type: "ai_assistant_plan",
              description: `Generated by AI assistant: ${activePlanModal.trigger || 'Manual'}`,
              actions: (activePlanModal.actions || []).map((act, i) => ({
                id: `act-${i + 1}`,
                type: "log_activity",
                config: { note: act }
              }))
